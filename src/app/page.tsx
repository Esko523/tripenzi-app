...
export default function TripenziApp() {
  ...
  const handleUpdateProfile = async () => {
      if(!currentUser) return;
      const { error } = await supabase.from('users').update({ name: editUserName, avatar: editUserAvatar }).eq('id', currentUser.id);
      if(!error) { const updatedUser = { ...currentUser, name: editUserName, avatar: editUserAvatar }; loginUser(updatedUser); setIsProfileOpen(false); }
  };

  const generateShareCode = () => {
      const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
      let result = ""; for (let i = 0; i < 6; i++) { if (i === 3) result += "-"; result += chars.charAt(Math.floor(Math.random() * chars.length)); } return result;
  };
  ...
  const handleAddTrip = async (e: React.FormEvent) => {
    ...
    if (!error && tripData) {
        ...
        await loadTrips(); setIsModalOpen(false); setNewName(""); 
