...
export default function WeatherWidget({ city }: WeatherProps) {
  ...
  useEffect(() => {
    ...
    const fetchWeather = async () => {
      try {
        ...
        // TADY JE ZMĚNA: Vytáhneme i country_code
        const { latitude, longitude, name, country_code } = geoData.results[0]; 
        
        // Nastavíme jméno jako "Město, CZ"
        if (country_code) {
  ...
  return (
    <div className="bg-white/20 backdrop-blur-md px-3 py-1 rounded-lg text-[10px] font-bold border border-white/20 flex items-center gap-2 text-white shadow-sm animate-in fade-in select-none">
      {getWeatherIcon(weatherCode || 0)}
      <span>{temp.toFixed(1)}°C</span>
      <span className="w-0.5 h-3 bg-white/30 rounded-full"></span>
      {/* truncate zajistí, že se dlouhý název "Mesto, XX" na mobilu zkrátí třemi tečkami */}
