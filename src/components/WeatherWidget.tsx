...
export default function WeatherWidget({ city }: WeatherProps) {
  ...
  useEffect(() => {
    ...
    const fetchWeather = async () => {
      try {
        ...
        const { latitude, longitude, name } = geoData.results[0]; // Bereme jen jméno města
        setFoundName(name);
  ...
  return (
    <div className="bg-white/20 backdrop-blur-md px-3 py-1 rounded-lg text-[10px] font-bold border border-white/20 flex items-center gap-2 text-white shadow-sm animate-in fade-in">
      {getWeatherIcon(weatherCode || 0)}
      <span>{temp.toFixed(1)}°C</span>
      {/* Oddělovač */}
      <span className="w-0.5 h-3 bg-white/30 rounded-full"></span>
      {/* Název města s ořezem pro mobily */}
      <span className="opacity-90 truncate max-w-[80px] sm:max-w-none">{foundName}</span>
    </div>
  );
}